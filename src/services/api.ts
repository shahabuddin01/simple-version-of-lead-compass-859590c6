...
export async function apiGetSMTPSettings() {
  return apiRequest('/settings/smtp');
}

export async function apiSaveSMTPSettings(settings: Record<string, any>) {
  return apiRequest('/settings/smtp', {
    method: 'POST',
...
export async function apiTestSMTP() {
  return apiRequest('/settings/smtp', {
    method: 'POST',
...
export async function apiGetSMSQueue(status?: string, limit = 50) {
  const params = new URLSearchParams();
  if (status) params.set('status', status);
  params.set('limit', String(limit));
  return apiRequest<any[]>(`/sms/queue?${params.toString()}`);
}

export async function apiAddToSMSQueue(data: {
  phones: string[];
  message: string;
  lead_id?: number;
  lead_name?: string;
  ...
  return apiRequest('/sms/queue', {
    method: 'POST',
    body: JSON.stringify(data),
  });
}

export async function apiUpdateSMSJob(jobId: number, action: 'cancel' | 'retry') {
  return apiRequest('/sms/queue', {
    method: 'PUT',
...
export async function apiGetSMSDevices() {
  return apiRequest<any[]>('/sms/device');
}

export async function apiDeactivateSMSDevice(deviceId: string) {
  return apiRequest('/sms/device', {
    method: 'DELETE',
    body: JSON.stringify({ device_id: deviceId }),
  });
}

export async function apiGetSMSStats() {
